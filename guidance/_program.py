...
class Program:
    """A program template that can be compiled and executed to generate a new
    filled in (executed) program.

    Note that as the template gets executed {{!-- handlebars comment
    markers --}} get left in the generated output to mark where template
    tags used to be.
    """
    ...
    def __aiter__(self):
        """Return an async iterator that yields the program in partial states
        as it is run."""
        return self._stream_run_async()

    def __call__(self, **kwargs):
        """Execute this program with the given variable values and return a new
        executed/executing program.
    ...
    def _stream_run(self, loop, new_program):
        """This feels a bit hacky at the moment.
...
<script type="text/javascript">{js_data}; window._guidanceDisplay("{self._id}");</script>"""
    ...
    async def execute(self):
        ...
        Note that as execution progresses the program will be
        incrementally converted from a template into a completed string
        (with variables stored). At each point in this process the
        current template remains valid.
        """
    ...
    def _build_html(self, text, last=False):
        ...
        if (
            re.search(
                r"{{!--GMARKER_START_(role|system|user|assistant|function)", display_out
            )
            is not None
        ):
            ...
            display_out = re.sub(
                r"\s*{{!--GMARKER_START_(role|system|user|assistant|function)\$(.*?)--}}",
            ...
            display_out = re.sub(
                r"{{!--GMARKER_END_(role|system|user|assistant|function)\$(.*?)--}}\s*",
                r"{{!--GMARKER_END_\1$\2--}}",
                display_out,
                flags=re.DOTALL,
            )
            ...
            display_out = re.sub(
                r"{{!--GMARKER_START_(role|system|user|assistant|function)\$(.*?)--}}"
                + "(.*?)"
                + r"{{!--GMARKER_END_(role|system|user|assistant|function)\$(.*?)--}}",
                role_box,
                display_out,
                flags=re.DOTALL,
            )
...
def add_spaces(s):
    """This adds spaces so the browser will show leading and trailing
    newlines."""
    if s.startswith("\n"):
        s = " " + s
    if s.endswith("\n"):
