...
class Seq2SeqTransformers(Transformers):
    def _model_and_tokenizer(self, model, tokenizer, **kwargs):
        if isinstance(model, str):
            ...
            if tokenizer is None:
                tokenizer = transformers.AutoTokenizer.from_pretrained(model, **kwargs)
            model = transformers.AutoModelForSeq2SeqLM.from_pretrained(model, **kwargs)
...
class TransformersSession(LLMSession):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        """Restore the model to its original state by removing monkey
        patches."""
...
class TokenHealingLogitsProcessor:
    ...
    When we tokenize the prompt the last token(s) we get are not the
    last token(s) we would have gotten if the prompt + generation was
    concatented and then tokenized. This is not good because it does not
    align with the pretraining of the model, so we "heal" this boundary
    by backing up as many tokens as needed and then forcing the first
    tokens generated to start with the prefix of the tokens we removed
    from the prompt. This could result in the same tokens at the end of
    the prompt, or some suffix of the tokens we removed could be
    replaced by a single longer one that crosses the prompt boundary.
    """

    def __init__(self, model, vocab_size, prompt_ids, bias_value=100.0):
        """Build a new TokenHealingLogitsProcessor.

        Note that bias_value is in score space (log-odds normally) and
        should be enough to ensure those tokens are the only ones used.
        """
...
class TransformersStringBuilder:
    """This deals with the complexity of building up a string from tokens bit
    by bit."""
...
class TransformersStreamer:
    ...
    def put(self, token_obj):
        ...
        # Outputs from generation function can interpreted differently depending on the model.
        ...
        if self.logprobs:
            assert (
                len(new_tokens) == 1
            ), "logprobs are not supported for batched generation right now in guidance.llms.Transformers"
            new_scores = [
