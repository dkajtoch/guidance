...
from .._utils import AsyncIter, escape_template_block
...
async def gen(
    ...
    **llm_kwargs
    ...
    if list_append:
        assert (
            name is not None
        ), "You must provide a variable name when using list_append=True"

    # if stop is None then we use the text of the node after the generate command
    if stop is None:
        next_text = (
            getattr(next_node, "text", next_node) if next_node is not None else ""
        )
        prev_text = (
            getattr(prev_node, "text", prev_node) if prev_node is not None else ""
        )
    ...
    if stream is None:
        stream = (
            ...
            or parser.program._displaying
            or stop_regex is not None
            if n == 1
            else False
        )
    ...
    assert (
        parser.llm_session is not None
    ), "You must set an LLM for the program to use (use the `llm=` parameter) before you can use the `gen` command."

    # call the LLM
    gen_obj = await parser.llm_session(
        variable_stack["@prefix"] + prefix,
        stop=stop,
        ...
        echo=parser.program.logprobs is not None,
        stream=stream,
        caching=parser.program.caching,
    ...
    if n == 1:
        ...
        logprobs_out = []
        if not isinstance(
            gen_obj, (types.AsyncGeneratorType, types.GeneratorType, list, tuple)
        ):
            gen_obj = [gen_obj]
        ...
        if list_append:
            ...
            if logprobs is not None:
                variable_stack[name + "_logprobs"] = variable_stack.get(
                    name + "_logprobs", []
                )
        ...
        generated_values = [
            prefix + choice["text"] + suffix for choice in gen_obj["choices"]
        ]
        if list_append:
            ...
            if logprobs is not None:
                variable_stack[name + "_logprobs"] = [
                    choice["logprobs"]["top_logprobs"] for choice in gen_obj["choices"]
                ]
        ...
        if not hidden:
            ...
            out = (
                "{{!--" + f"GMARKERmany_generate_start_{not hidden}_{l}${id}$" + "--}}"
            )
            for i, value in enumerate(generated_values):
                ...
                if i > 0:
                    ...
                    out += value
            variable_stack["@raw_prefix"] += (
                out + "--}}{{!--" + f"GMARKERmany_generate_end${id}$" + "--}}"
            )
            return
