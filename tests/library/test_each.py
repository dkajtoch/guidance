...
def test_each():
    ...
    assert (
        str(prompt(name="Guidance", names=["Bob", "Sue"])) == "Hello, Guidance! Bob Sue"
    )
...
def test_each_with_objects():
    ...
    out = prompt(name="Guidance", names=[{"name": "Bob"}, {"name": "Sue"}])
...
def test_each_after_await():
    ...
    assert (
        str(prompt(name="Guidance", names=["Bob", "Sue"]))
        == "Hello, Guidance!{{await 'some_var'}}{{#each names}} {{this}}{{/each}}"
    )
...
def test_each_over_an_await():
    ...
    assert (
        ...
        == "Hello, Guidance!{{#each (await 'names')}} {{this}}{{/each}}"
    )
...
{{/each}}""",
    ...
    assert (
        ...
        == "Hello, Guidance! Here are 5 names and their favorite food:\n"
    )
...
def test_each_parallel_with_gen_openai():
    """Test an each loop run in parallel with generations inside using
    OpenAI."""
