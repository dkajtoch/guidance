...
def test_multi_token_healing(llm):
    """Test if we can heal prompt boundaries where we need to back up two
    tokens."""
...
def test_stop_regex_cut_short(llm):
    """Test that the stop_regex argument works as expected even when max_tokens
    cuts it short."""
    llm = get_llm(llm)
    out = guidance(
        """Repeat this ten times: "s38 kdjksid", "s38 kdjksid", "s38 kdjksid", "s38 kdjksid", "{{gen 'text' stop_regex="s38 kdjksid" max_tokens=5 save_stop_text=True}}""",
        llm=llm,
    )()
    assert (
        len(out["text"]) > 0
