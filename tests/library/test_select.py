...
def test_select_names(llm):
    ...
    llm = get_llm(llm)
    out = guidance("Hello, {{#select 'name'}}Alice{{or}}Bob{{/select}}", llm=llm)()
    assert out["name"] in ["Alice", "Bob"]


def test_select_multi_path():
    """Test the behavior of `select` and confirm the returns probability
...
def test_non_greedy_tokenize(llm):
    """Test the behavior of `select` when the GPT tokenizer is not greedy (odd
    space handling)."""
